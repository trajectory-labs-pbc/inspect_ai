...
async def test_task_sessions_are_instance_level() -> None:
    """Two MCPServerLocal instances share no session state, even on the
    same anyio task (i.e. same task id).
    """
    a = _make_server("a")
    b = _make_server("b")
...
async def test_task_sessions_independent_across_instances_with_same_name() -> None:
    """Two instances sharing the same ``name`` (a common pattern when the
    same Task builder runs for every sample) don't collide.
    """
...
async def test_cached_tool_list_cleared_on_close() -> None:
    """When a session's refcount drops to zero, its cached tool list must
    be cleared. Otherwise a reused session object would serve stale tools
    to the next caller without contacting the server.
    """
...
async def test_cached_tool_list_preserved_when_still_referenced() -> None:
    """If the session is still held by another caller (refcount > 0),
    closing one holder must NOT invalidate the cache — the remaining
    holder still expects its tools.
    """
    session = MCPServerLocalSession(
        client=lambda: (_ for _ in ()).throw(AssertionError("unused")),
        name="s",
