...
| `INSPECT_GC_MODE` | Opt-in operational garbage-collector policy. Values: `default`, `freeze`, `low_latency`. `low_latency` suppresses automatic full (generation-2) collections and requires a finite cgroup memory limit for its high-water guard. |
