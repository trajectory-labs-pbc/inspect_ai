    ...
    async def __call__(
        ...
        **transport_extra_args: Any,
        ...
        for attempt in range(_MAX_RETRIES + 1):
            ...
            # MCP server inside the container is unaffected. Prefer
            # stderr, but fall back to stdout — some failures (e.g. MCP
            # server crash, entrypoint error) only surface in stdout
            # because the sandbox CLI wrote its diagnostic there.
            error_detail = (
                exec_result.stderr
            ...
            last_error = RuntimeError(
                ...
                f"{rpc_call_description(method, params)}: "
                f"{error_detail}"
            )
