...
class MCPServerLocal(MCPServer):
    def __init__(
        ...
        *,
        ...
        # object. Storing it on the instance (rather than the class) means
        # one sample's cached session can't leak into another. Weak keys
        # mean an entry dies with its key (sample or task) instead of
        # needing manual eviction, and — unlike a raw anyio task id, which
        # is id()-derived and reusable once its task is GC'd — the task
        # object itself is never mistakenly reused for an unrelated task.
    ...
    def _session_scope(self) -> object:
        ...
        current task is the scope, preserving the previous behaviour. Tool
        calls within one sample can still run concurrently (parallel
        tool-call stages), and a handoff/subagent's entire turn — including
        its own tools() resolution — runs inside its own child task; those
        share the sample's ActiveSample scope rather than a per-task one, so
        a handoff adopts the parent's live session (see
        ``_current_scope_session``) instead of always starting fresh.
        """
